...
if [ $(grep -c Intel /proc/cpuinfo) -gt 0 ]; then
    ...
    /home/mev/source/wspy/topdown -o topdown.txt --topdown2 --no-rusage --no-software -no-ipc phoronix-test-suite batch-run stream
    /home/mev/source/wspy/topdown -o ipc.l2.txt --ipc --cache2 --no-software --no-rusage phoronix-test-suite batch-run stream
    cat software.branch.txt topdown.txt ipc.l2.txt > topdown.txt
else
