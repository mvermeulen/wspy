...
if [ $(grep -c Intel /proc/cpuinfo) -gt 0 ]; then
    /home/mev/source/wspy/topdown -o software.branch.txt --rusage --software --no-ipc --branch phoronix-test-suite batch-run coremark 2>&1 | tee coremark.out
    ...
    /home/mev/source/wspy/topdown -o treerun.txt --tree process.tree.txt --software --no-ipc phoronix-test-suite batch-run coremark 2>&1 | tee build-gcc-tree.out
    /home/mev/source/wspy/topdown -o software.branch.txt --rusage --software --branch --no-ipc phoronix-test-suite batch-run coremark 2>&1 | tee coremark.out
