...
if [ $(grep -c Intel /proc/cpuinfo) -gt 0 ]; then
    ...
    /home/mev/source/wspy/wspy -o tlbcache.txt --topdown-frontend --topdown-optlb --no-ipc --no-software --no-rusage batch-run $TESTNAME
    ...
    cat software.branch.txt ipc.topdown.txt l2.float.txt tlbcache.txt > amdtopdown.txt
