...
- **GPU multi-device enumeration:** `--gpu-busy`/`--gpu-metrics`/`--gpu-smi`/`--gpu-device=<idx>` on an
  ...
  (`gpu-compute-profile`'s real yquake2/ollama testing validated single-GPU correctness, not
  multi-device selection.)
- **`wspy-validate`/`wspy-ledger` at accumulated real scale:** the orphaned-run-index-record case (PR
  #81, see "Shipped since 4.1") was one concrete real-world messiness case found and fixed; interrupted
  runs and mixed-schema-version behavior over a long-lived, real run-index file still haven't been
  exercised beyond `test_ledger.c`/`test_validate.c`'s small synthetic fixtures.
...
1. `--gpu-smi --interval` CSV column-count gap: `timer_callback()` (`topdown.c`) never reads
   `amd_smi` per tick at all, only in aggregate mode, so a periodic `--gpu-smi --interval` row is
   missing 4 columns the header claims. Doesn't affect `gpu-compute` (uses `--gpu-metrics`/
   `--gpu-busy`/`--gpu-nvidia`, not `--gpu-smi`).
2. A rare pre-existing race: when the last `--interval` tick lands almost exactly at child exit, the
...
3. `deep-gpu`'s `wspy-run` systemtime pass doesn't carry `--power` the way `deep-cpu`'s does — a
   pre-existing asymmetry between the two profiles.
4. The web launcher's custom-checklist GPU card has no `--gpu-nvidia` checkbox — only AMD's
...
**Tier 2 — real-hardware validation carried forward (no code change, just testing — see "Known gaps"
above for detail):**

5. Compare AMD IBS filtered vs. unfiltered sample distributions on real hardware.
6. Exercise GPU device enumeration/selection on a real multi-GPU host.
7. Exercise `wspy-validate`/`wspy-ledger` against a real run-index file with interrupted runs and
   mixed schema versions accumulated at scale.

**Tier 3 — stats/confidence layer:**

8. Repeatability policy + confidence metadata (mean, stddev, CV, CI) as default output.
9. Comparison matrix mode (sweep compiler/kernel/governor/SMT/VM-native) — builds on the
   profile-driven launcher; a declarative sweep runner, not new collection logic.

**Tier 4 — topdown/IBS refinement (interdependent; sets up 4.3's attribution work):**

10. Hierarchical (parent→child) topdown schema + explicit raw-vs-contention-adjusted denominators +
    formula/version metadata.
11. Core-class-aware topdown (hybrid Intel Atom+Core; weighted aggregate) — depends on per-core
    collection (shipped) plus the hierarchical schema above.
12. Zen-family preset packs (`zen-portable`, `zen4plus-deep`) — convenience layer now that IBS
    capability probing exists.
13. PMU-capability-aware comparability warnings.
14. AMD IBS *sampling*-mode support: mmap'ing the perf ring buffer and requesting `PERF_SAMPLE_RAW`
...
15. Per-core energy (`power_core`) support: `--power` currently reports package-level `pkg_joules`/
...
16. ROCm SMI + sysfs fusion layer (one stream, source precedence, per-metric validity flags) —
    merges the two existing independent GPU paths (`amd_smi.c`, `amd_sysfs.c`).
17. Same manifest/index/profile pipeline extended to GPU runs (busy/clocks/power/temp/memory
...
**Tier 6 — `/proc` and tree enrichment remainder (independent, moderate value, low risk):**

18. cgroup identity + limits in manifest, `cpu.stat` throttling stats — needed for fair comparison in
    containerized environments.
19. Per-core (`--per-core`) → imbalance/hot-core/migration diagnostics, core-class summaries.
20. `proctree` → JSON/Graphviz export + run-to-run tree diff.

**Tier 7 — characterization prerequisites:**

21. Feature normalization prerequisites (fixed feature set from counters/topdown/faults/context-
    switch/I-O) — needs 4.1's normalized store schema (`wspy-store`) to draw features from.
22. Archetype scorecard (parallelism shape, resource dominance, control-flow style, runtime
    stability) + confidence + top-2 alternatives.

**Tier 8 — launcher/infra follow-ups:**

23. Collapse `wspy-run`'s builtin profiles (`deep-cpu` et al.) onto native `--passes` bin-packing.
...
24. Job-browsing view in the web UI. A queued job (`wspy-queue add`, or the Run tab's "Queue instead
...
25. Give the report compare view (`GET /compare`) its own curation/annotation layer. It's deliberately
...
**Tier 9 — docs/testing/release process:**

26. Profile cookbook + interpretation playbook (how to read confidence/phase/comparability/cluster
    output).
27. Reproducibility bundle export (tarball: manifest + raw + derived per batch).
28. Size `wspy-run`'s `--tree` pass timeout from an actual run-time estimate instead of a fixed 3600s
...
29. Doc/version consistency check — an automated check (script, or an addition to `run_tests.sh`)
    ...
    every release again.
30. Release-prep checklist/script — capture the v4.0 release process (bump `WSPY_VERSION_MAJOR`/
