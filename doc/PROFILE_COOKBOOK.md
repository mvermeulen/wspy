...
the `allocation_pressure` axis (issue #231) and `memory_attribution`/`memory_attribution_locus` (4.3
...
- Otherwise, confidence is driven by **margin** (the gap in percentage points between the winning
  ...
  `parallelism_shape`/`control_flow_style`/`runtime_stability`/`allocation_pressure` — had data):
...
- **`missing-<axis>-data`** (one per unavailable supporting axis) — that axis needs a flag this run
  ...
  `runtime_stability` needs `--interval`; `allocation_pressure` needs only rusage, which every run
  always collects, so it's rarely the missing one. **What to do**: if you want a `high`-confidence
...
captured before the `allocation_pressure` axis (issue #231) existed, so this row's columns predate it
...
- **`allocation_pressure`** (`low`/`moderate`/`high`, needs only rusage — always collected, so this
  axis is rarely `unknown`) — threshold on `fault_rate` ((`minflt`+`majflt`)/`elapsed_seconds`);
  `high` corroborates a `memory-bound`/`compute-bound` read that's actually dominated by dynamic
  allocation/deallocation overhead rather than the workload's own core computation (issue #231's GCC-
  optimization-guide motivation: heavy minor-fault rates favor static allocation/memory pooling
  remediation over other tuning).
