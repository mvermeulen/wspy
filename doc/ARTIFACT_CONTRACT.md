...
**The eight axes**, each independently `unknown`/`insufficient-data` when its source `run_features`
...
- **`vectorization_density`** — `low`/`moderate`/`high`, from `float_pct` (AMD only, needs `--float`).
  Added in issue #227; see `doc/METRICS.md`'s entry for the threshold values.
- **`core_utilization`** — `low`/`moderate`/`high`, from `on_cpu` (rusage + manifest, always
  collected). Added in issue #230's first half; see `doc/METRICS.md`'s entry for the threshold values
  and why the issue's other half (an oversubscription-specific label) isn't the same recipe.
- **`memory_attribution`**/**`memory_attribution_locus`** — not a threshold on a single value like the
  seven axes above; cross-references `backend_pct` against every independently-measured cache/TLB/IBS
...
`>= 20` points, `medium` needs `>= 10`) and how many of the 6 supporting axes (`parallelism_shape`/
`control_flow_style`/`runtime_stability`/`allocation_pressure`/`vectorization_density`/
`core_utilization`) had data (`high` needs `>= 2` known, `medium` needs `>= 1`); `narrow-margin`
appears whenever the margin missed the `high` threshold, and one `missing-<axis>-data` reason per
unavailable supporting axis, so a close call is distinguishable from missing data as the reason
confidence isn't `high`. `memory_attribution`/`memory_attribution_locus` don't feed into this count —
they're independent corroboration, not confidence inputs.
...
- Default (`score_runs()`) scores every run matching `--command`/`--hostname` filters, one row per run
  ...
  alternative_pct,parallelism_shape,control_flow_style,runtime_stability,allocation_pressure,
  vectorization_density,core_utilization,confidence,confidence_reasons,memory_attribution,
  memory_attribution_reasons,memory_attribution_locus,memory_attribution_locus_reasons`, or a
  fixed-width human table) — an `INNER JOIN` against `run_features`, so a
...
**Don't glob for `manifest.json` to find something to pass to `wspy-validate`.** The natural first
attempt at automating "validate this run" is `wspy-validate $(find <rundir> -name '*manifest*.json')`,
but that also matches the run-level `manifest.json` above, which isn't a real per-process wspy
manifest at all — `wspy-validate` reports a spurious `FAIL: no schema_version field -- doesn't look
like a wspy manifest` against it. Use `passes[].manifest` (below) to enumerate the real ones instead.
