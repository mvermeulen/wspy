...
| File | Responsibility |
|---|---|
| `wspy.c` | `main()`, `parse_options()` (flags into globals: `counter_mask`, `system_mask`, `aflag`/`sflag`/`csvflag`/`treeflag`), orchestrates setup → start → read → print. `--passes=<list>` does native multi-pass counter execution (`multipass.c`); `--affinity=<spec>` (`affinity.c`) resolves placement pre-launch; `--counters=<list>` replaces the ~20 per-group boolean flags (soft-deprecated). `--list-columns` (INVESTIGATION.md's 4.4(a) "Preset/Configuration/Option vocabulary refactor" item) prints the CSV header a given flag combination would produce and exits before ever launching a workload — it falls through the exact same single-pass/`--passes` group-construction and header-printing code a real run uses (an early return right after each path's own CSV-header block, rather than a separate hand-maintained probe function) so it can never drift from what a real run actually emits; forces `csvflag` on. `web/joblib.py`'s `profile_plottable_columns()` is its first caller. |
...
| `archetype.c` | `wspy-archetype`: classifies a run along 4 axes (`resource_dominance` top-2 ranked, plus `parallelism_shape`/`control_flow_style`/`runtime_stability`) with overall confidence. See `doc/PROFILE_COOKBOOK.md`. `--run-guest <json>` (4.3 item 23) scores a flat `feature_name`→value JSON object with no database at all, via the same pure `score_snapshot()` — used by `wspy-testpoint render`'s characterization section to score WordPress-recovered machines (item 21) that have no local store presence. |
...
| `multipass.c`/`multipass.h` | Testable half of `--passes`: bin-packs counter groups into budget-respecting passes, or one oversubscribed pass (`--multiplex`). `COUNTER_IBS` excluded. Also owns `multipass_group_names[]`, the canonical `--counters=<list>`/`--passes=<list>` token vocabulary; `wspy --list-groups` (standalone, no privileges) dumps it machine-readably — `tests/group_vocab_check.sh` checks `web/joblib.py`'s `ALL_GROUPS` against it. |
...
| `wspy-run` | Bash profile-driven launcher (not in the C build). Runs named "passes" from a builtin profile (`quick`, `deep-cpu`, `deep-cpu-intel`, `deep-gpu`, `tree-heavy`, `ibs-basic`, `ibs-memory-deep`, `ibs-sample`, `gpu-compute`, `zen-portable`, `zen4plus-deep`) or a `-c <file>` list; `--suite`/`--benchmark` writes the unified `<outdir>/<suite>/<benchmark>/<run-id>/` layout. Each pass is its own `wspy` invocation; `--affinity`/`--config-option` pass through to all of them. Builtin profiles are data, not code — `profiles/*.conf` (plain pass lists) and `profiles/*.spec` (composites, e.g. `zen-portable`/`zen4plus-deep`), both read by `load_builtin_profile()`; `web/joblib.py`'s `COMPOSITE_PRESET_PROFILES` reads the same `*.spec` files rather than hand-mirroring them. |
| `wspy-sweep` | Python sweep runner. Cross-products axis handlers (v1: only `"affinity"`) against workload commands via `wspy-run`, tagging each cell's swept value for later `--group-by-option` analysis. Each cell picks its own instrumentation the same two mutually-exclusive ways `wspy-run` itself does — `"profile"` (builtin preset name) or `"config_file"` (a `-c`/`--config` pass-list file) — in either the quick-form CLI (`--profile`/`--config`\|`-c`) or a declarative `--spec` JSON. `--summary-out <file>` writes a JSON list of `{cell_id, workload_name, run_id, rundir, exit_code}` after a real (non-dry-run) sweep, letting a caller (`wspy-phoronix-batch`) discover exactly which `run_id`/`rundir` each of its own workloads landed at without scraping stdout. |
...
| `wspy-phoronix-segment` | Python telemetry segmentation (`doc/phoronix_hook_investigation.md` for the full investigation/design writeup). Partitions a `--interval` wspy CSV into per-test-case datasets by correlating each Phoronix test-option's real execution window against wspy's own "time" column — no ptrace/process-tree tracing needed. Two correlation sources per pass, tried in order: a `<pass-name>.pts_hooks.log` artifact (`manifest.json`'s `passes[].pts_hooks_log`, written by `scripts/pts_hooks/{pre,post}_test_run.sh` via PTS's `result_notifier` hooks — preferred, no `composite.xml` hash-mapping indirection needed, but only per-test-option granularity since `__pre_test_run`/`__post_test_run` fire once per option, not per individual trial); falling back to `composite.xml` + per-hash `.log`-file timestamp correlation (real per-trial granularity, via `pts-install.json`'s hash↔description mapping) when a pass has no hook log. Supersedes the `scripts/wspy-phoronix-segment.py` prototype (removed). |
| `wspy-phoronix-batch` | Python batch runner for many already-materialized Phoronix test points (`workload/phoronix/<test>/<options>/`) in one non-interactive call — until now the only way to actually run one was the web launcher's "Use in Run tab" button, one point at a time, by a human clicking Run. `--point <identity>` (repeatable) / `--points-file <file>` (one identity per line, `#`-comments/blanks ignored) / `--all` (every point materialized under `--dest`) select which points to run; an identity that doesn't resolve is skipped with a warning, never fatal for the rest of the batch. Doesn't reimplement execution: builds a `wspy-sweep --spec` JSON (one `workloads[]` entry per point, each `command` a `phoronix-test-suite batch-run local/<identity>`, `profile`/`config_file` passed straight through from its own `--profile`/`-c`/`--config`) and delegates to `wspy-sweep` for the actual cross-product/dry-run/failure-tracking/store-ingest loop. After a real (non-dry-run) run, reads `wspy-sweep --summary-out`'s JSON to learn each point's real `run_id`/`rundir` and symlinks it back into that point's own `runs/` directory (`web/joblib.py`'s `link_phoronix_test_point_run()`, `--no-link-back` to skip), restoring "Use in Run tab"'s single-run browsability for a whole batch. |
| `web/` | `server.py`/`joblib.py`: stdlib `ThreadingHTTPServer` (`127.0.0.1` default) — tabbed Run/Validate/Store & Summary/Discovery launcher plus a report browser with curation, cross-run compare, tree viewer/diff (`proctree_viewer.js`, `/tree-viewer/...`, single-tree mode has its own Tree/Timeline view-mode switch — the latter a swimlane-by-process-start/finish rendering of the same fetched tree, no `--interval` dependency at all, coloring a `--target`-matched process's bar by its own Retiring % where available), an interactive `--interval` timeline viewer, a combined tree+timeline swimlane view (`timeline_viewer.js`, `/timeline-viewer/...`, only linked when `joblib.find_combined_timeline_csv()` finds a pass whose own wspy manifest recorded both `--tree` and `--interval` together — the two artifacts only share one clock origin when produced by the same invocation), and publish-ready export. `joblib.py` holds argv builders/executors shared with `wspy-queue`; see in-code docstrings for the endpoint list. `report_root.py` and `wp_client.py` are thin shared clients (git and WordPress REST respectively) used by `wspy-publish`/`wspy-testpoint`. `machine_registry.py` (4.3 Tier 3 item 5) is a read-only hostname→machine-slug lookup, scanned from `scripts/publish_machine_page.py`'s existing `<report-root>/machine/<short-name>/machine.json` catalog — it never writes; the catalog script is the only place a hostname/slug association is recorded. `counter_text.py` (4.3 item 21) parses wspy's human-readable `counters.txt`/`ibs.txt` output into structured metric records; `recover_machine_metrics_from_wordpress()` (`server.py`) uses it plus `wp_client.extract_preformatted_blocks()` to recover real metric values for a machine with no local `runs.json`/store presence directly from that machine's own already-published WordPress pages, merged into the reference-matrix detail page as a clearly-marked, lower-trust fallback column (real local data always wins when both exist). The report page's "Publish test-point report" card (`render_testpoint_card()`, Tier 3 item 7 / 4.4(a) "One-click end-to-end pipeline") runs a best-effort store re-ingest (`run_store_ingest_besteffort()`, `joblib.py` — catches up a run launched from a bare `wspy-run` that skipped the automatic ingest a web-launched run already gets) then `wspy-testpoint select-runs` + `render` as one background-thread subprocess pipeline (`execute_testpoint_publish()`, its own `TESTPOINT_RUNS` SSE registry, same shape as the "AI narrative analysis" card's `ANALYZE_RUNS`), optionally followed by `report_root.push_report_root()` (git push to the report-root's remote) if a human explicitly checked "also push" — defaulting to `git push --dry-run` (a real dry-run, not a hand-rolled preview) even then, so a real push needs two separate opt-ins, not one; never wired into `wspy-queue` or any other automated path. `--report-root`/`--report-root-remote` CLI flags override the whole server instance's target clone (unset by default, so it falls through to `wspy-testpoint`'s own real-deployment default chain). |
...
- **New `wspy-run` builtin profile:** add a `profiles/<name>.conf` file (one `<pass-name>
  <wspy-flags...>` line per pass, `#`-comment lines allowed, `${OUTDIR}`/`${PREFIX}` tokens
