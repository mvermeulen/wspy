...
- `make` — builds `wspy`, `cpu_info`, `proctree`, `wspy-validate` (default, no GPU support)
...
- `make test` — builds and runs `test_wspy`, `test_proctree`, and `test_validate` (unit-style tests; `test_wspy.c` `#include`s `wspy.c` directly with `TEST_WSPY`/`AMDGPU=0` defined to stub out `main()` and disable GPU code — the `test_wspy` Makefile target always compiles its own objects with `-DAMDGPU=0`, independent of whatever the top-level `AMDGPU` value is, so it can't pick up a mismatched GPU-enabled `topdown.o` from the same build tree; `test_validate.c` follows the same `#include "validate.c"` + `-DTEST_VALIDATE` pattern to reach `validate.c`'s static check functions)
