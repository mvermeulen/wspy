...
- `ibs.c`/`ibs.h` — capability-driven discovery of AMD's Instruction-Based Sampling (IBS) PMU support (`INVESTIGATION_4.0.md`'s "Zen5 / IBS" track, "Capability-driven IBS probing" item — the named prerequisite for the rest of that track). IBS is exposed as two dynamic PMUs the kernel registers at boot, `ibs_fetch`/`ibs_op`, present only when the CPU/kernel combination supports it; `ibs_probe()` reads `/sys/bus/event_source/devices/{ibs_fetch,ibs_op}/{type,format/*,caps/*}` at runtime (readdir-driven, not a hardcoded field list) rather than assuming which format fields (e.g. `l3missonly`, `ldlat`, `rand_en`) or caps (e.g. `zen4_ibs_extensions`, `dtlb_pgsize`) exist, since both vary by kernel version and CPU generation — exactly the "avoids hardcoding assumptions that break on non-Zen5 kernels" the inventory row calls for. Returns a `struct ibs_capabilities` (never fails; an absent PMU just yields `present`/`supported` 0) that `wspy --capabilities` prints via `print_ibs_capability_report()` alongside the existing counter capability report. `ibs_pmu_format()`/`ibs_pmu_cap()` are lookup helpers for a future caller (the separate, not-yet-built "`ibs-basic`/`ibs-memory-deep` collection profiles" item) that needs to check a specific field/cap by name. Deliberately scoped to discovery only — it doesn't build `perf_event_attr` configs or open any IBS counters.
