...
fallback (4.1 item #7) is this rule, discovered bottom-up from the real constraints before being named
...
Implications:
- **Web launcher (#7):** read as *presets first* — named shortcuts that populate a configuration+
...
- **Reports (#14/#15, new):** a report today only records a flat command line and flag set. Recording
...
→ Informs 4.1's #7 (web launcher — presets expanding to configurations+options), #14 (structured
configuration provenance), and #15 (report ↔ configuration linkage + customize-and-rerun). Also
...
5. Design/mockup pass for the web interface, done deliberately *before* building #6-12 below — the
   "step back" this reorg calls for. Covers both halves: wireframes/mockups for the input side (how
   to organize forms across more than just `wspy-run` — see #7) and the output side (a single-run
   report page, a compare-two-or-more-runs view, and a historical index/search view — see #6/#9).
   Explicitly evaluate static-file-only rendering (works from `file://` or any static host, no
   backend process) against a thin dynamic backend, since that choice shapes #6's report studio, #7's
   launcher, and #8's export format all at once. Deliverable is throwaway mockups plus a short
   writeup of the chosen direction and open tradeoffs, not production code — expect #6-12 to be
   ...
   converged on: per-tool tabs at the top level (#7) with a *multi-select* capability checklist inside
   ...
   local-vs-shared deployment toggle answering #11 directly (local executes with live output, shared
...
6. Report review + curation studio (supersedes the original "HTML report bundle" framing — same item
   ...
   **curate** — select a subset, reorder it, and write commentary *per configuration* ("what does
   this tell us"), not just one whole-report note, then hand it off to #8's export. A configuration
...
7. Web-based run launcher + report browser — a thin web form over `wspy-run`'s option surface
   (profile/suite/benchmark/workload command) that constructs and executes the command, then links
   to the resulting output once it lands. Most valuable once #6's report studio exists to link
   ...
   territory.
8. Publish-ready data export format — takes #6's curated block sequence (heading/image/preformatted/
...
9. Historical run index browser/search.
10. Shared plotting templates — replace `workload/phoronix/gnuplot.sh`'s per-suite script with one
    normalized-schema pipeline once #1 exists. This is what closes #8's real-image gap: the studio
    (#6) and export (#8) currently stand in a placeholder for every chart because nothing yet
    generates the actual `plots/*.png` a real report would reference — this item is that generator,
    not a separate concern from the reporting work above.
11. Deployment/hosting design note — answer, for both a person browsing their own local run output
    and a team publishing to a shared site: does #6/#9 need to run anywhere besides `localhost`, is a
    static site (generated files, no server process) sufficient, and if not, what's the smallest
    backend that covers both cases? Feeds #5's mockup pass directly and should land before #7's
    launcher decides how it invokes `wspy-run` (local subprocess vs. something that only makes sense
    on a single machine).
12. Traceability links (summary row → manifest → raw CSV → plots → tree artifacts) — closes the
...
13. Report commentary/annotation — free-text notes saved alongside a report (session-local in the
    ...
    ("what does this tell us," attached to that configuration's block in #6) plus one overview note for
...
14. Structured configuration provenance — record a run's preset/configuration/option choices (see the
    deep-dive above) as structured data in `manifest.h`/`run_index.h`, not just the flat command line
    and flag set already captured. This is what lets #6/#9's reports render "how this was run" in the
    same vocabulary the launcher uses, and lets #15's "customize & run again" restore exact launcher
...
15. Browse-reports: relate each report's artifacts back to the preset/configuration/option choices
    that produced it (via #14), and add a "customize & run again" action that returns to the launcher
    ...
    version depends on #14 actually persisting that state somewhere reports can read it back from.
...
8. Static-site publishing pipeline (per-benchmark + suite + cross-suite pages from templates). Distinct
   from 4.1 #6's per-run studio, not a replacement for it: the studio is where one report gets
   curated by a person; this is what turns *many* already-curated (or un-curated, template-driven)
   reports into a browsable site. Likely consumes the same export formats #8 (4.1) produces rather
   than inventing a fourth.
9. Characterization badges + similarity panels in reports — a new block type in 4.1 #6's studio once
   4.2 #18's archetype scorecard exists to draw a badge from, not a separate report surface.
10. Interactive tree/timeline drill-down, GPU phase overlays — the interactive counterpart to 4.1 #6's
...
- **Should `wspy-run`'s builtin profiles be refactored to be declaratively defined (as
  ...
  Recommendation: not yet — let the web UI's preset/configuration/option model (4.1 #7) stabilize
