...
struct system_state {
  ...
  struct cpustat {
    unsigned long usertime, systemtime, idletime, irqtime;
    unsigned long last_usertime, last_systemtime, last_idletime, last_irqtime;
    unsigned long prev_usertime, prev_systemtime, prev_idletime, prev_irqtime; } cpu;
} system_state = { 0 };
...
void print_system(enum output_format oformat){
  ...
  case PRINT_CSV_HEADER:
    ...
    if (system_mask & SYSTEM_CPU) fprintf(outfile,"cpu,idle,irq,");
    break;
  case PRINT_CSV:
