...
SRCS = wspy.c cpu_info.c error.c json_util.c json_reader.c manifest.c run_index.c coverage.c provenance.c ibs.c proctree.c system.c topdown.c amd_smi.c amd_sysfs.c validate.c
OBJS = wspy.o cpu_info.o error.o json_util.o json_reader.o manifest.o run_index.o coverage.o provenance.o ibs.o proctree.o system.o topdown.o amd_smi.o
LIBS = -lpthread -lm
...
wspy:	wspy.o topdown.o error.o system.o json_util.o manifest.o run_index.o coverage.o provenance.o ibs.o cpu_info.c cpu_info.h
ifdef AMDGPU
	$(CC) -o wspy $(CFLAGS) wspy.o topdown.o cpu_info.c amd_smi.c amd_sysfs.c error.o system.o json_util.o manifest.o run_index.o coverage.o provenance.o ibs.o $(LIBS)
else
	$(CC) -o wspy $(CFLAGS) wspy.o topdown.o cpu_info.c error.o system.o json_util.o manifest.o run_index.o coverage.o provenance.o ibs.o $(LIBS)
endif
...
wspy.o: wspy.h cpu_info.h error.h manifest.h run_index.h coverage.h provenance.h ibs.h
...
test_wspy: test_wspy.c wspy.c wspy.h cpu_info.h error.h manifest.h manifest.c run_index.h run_index.c json_util.h json_util.c coverage.h coverage.c provenance.h provenance.c ibs.h ibs.c
	...
	$(CC) -o test_wspy -g -DAMDGPU=0 -DTEST_WSPY test_wspy.c test_error.o test_cpu_info.o test_system.o test_topdown.o test_json_util.o test_manifest.o test_run_index.o test_coverage.o test_provenance.o test_ibs_probe.o -lpthread -lm
