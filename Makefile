...
SRCS = wspy.c cpu_info.c error.c json_util.c json_reader.c manifest.c run_index.c coverage.c provenance.c ibs.c power.c preflight.c phase.c multipass.c affinity.c proctree.c system.c topdown.c amd_smi.c amd_sysfs.c validate.c
...
ifdef AMDGPU
	$(CC) -o wspy $(CFLAGS) wspy.o topdown.o cpu_info.c amd_smi.c amd_sysfs.c error.o system.o json_util.o manifest.o run_index.o coverage.o provenance.o ibs.o power.o preflight.o phase.o multipass.o affinity.o $(LIBS)
else
	$(CC) -o wspy $(CFLAGS) wspy.o topdown.o cpu_info.c error.o system.o json_util.o manifest.o run_index.o coverage.o provenance.o ibs.o power.o preflight.o phase.o multipass.o affinity.o $(LIBS)
endif
...
clobber:	clean
	-rm wspy cpu_info amd_smi amd_sysfs proctree wspy-validate wspy-ledger wspy-store wspy-summary wspy-plot test_hip_init test_hip_kernel test_proctree test_wspy test_validate test_ledger test_ibs test_power test_phase test_store test_summary test_plot test_affinity libwspy_profiler.so

# DO NOT DELETE

wspy.o: wspy.h cpu_info.h error.h manifest.h run_index.h coverage.h provenance.h ibs.h power.h preflight.h phase.h affinity.h
...
test_wspy: test_wspy.c wspy.c wspy.h cpu_info.h error.h manifest.h manifest.c run_index.h run_index.c json_util.h json_util.c coverage.h coverage.c provenance.h provenance.c ibs.h ibs.c power.h power.c preflight.h preflight.c phase.h phase.c multipass.h multipass.c affinity.h affinity.c
	...
	$(CC) -g -DAMDGPU=0 -c -o test_power_probe.o power.c
